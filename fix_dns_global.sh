#!/usr/bin/env bash
# 文件名: fix_dns_global.sh
# 适用于: 国外 VPS (香港、美国、欧洲等)
set -euo pipefail

# --- 核心配置: Google & Cloudflare ---
readonly TARGET_DNS="8.8.8.8#dns.google 1.1.1.1#cloudflare-dns.com 2606:4700:4700::1111#cloudflare-dns.com 2001:4860:4860::8888#dns.google"
# ------------------------------------
...
main() {
    ...
    if ! command -v resolvectl &> /dev/null || ! resolvectl status &> /dev/null; then
        ...
        # 宽松匹配：只要包含主要的目标DNS即可，防止因顺序不同导致判断失败
        if [[ "${current_dns}" != *"${TARGET_DNS%% *}"* ]] || ! echo "${status_output}" | grep -q -- "+DNSOverTLS" || ! echo "${status_output}" | grep -q "DNSSEC=allow-downgrade"; then
